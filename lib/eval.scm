...
(define (comp-quasiquotation form level env)
  (cond ((= level 0)
        ...
        ((vector? form)
         (gen-vector-form
           (comp-quasiquotation-list (vector->list form) level env)))
        (else
         (gen-cst form))))
...
(define (comp-define-macro expr env)
  (let ((x (definition-name expr)))
    (gen-macro x (eval (definition-value expr)))))
